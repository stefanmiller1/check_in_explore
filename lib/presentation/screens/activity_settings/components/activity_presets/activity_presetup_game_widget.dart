...
class ActivitySetupGames extends StatelessWidget {
  ...
  const ActivitySetupGames({Key? key, required this.model, required this.activityCreatorForm}) : super(key: key);
                  ...
                  Row(
                    children: [
                      Container(
                          ...
                          width: 60,
                          child: Center(
                            child: Text(context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.gameActivityAvailability?.tournamentNumberOfTeams.toString() ?? '4', style: TextStyle(color: model.disabledTextColor)
                            ),
                          )
                      ),
                      QuantityButtons(
                          model: model,
                          initNumber: context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.gameActivityAvailability?.tournamentNumberOfTeams ?? 4,
