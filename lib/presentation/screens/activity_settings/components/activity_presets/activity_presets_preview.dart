  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Column(
        ...
        children: [
          if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.isWorkingAlone == null)
            Text('Add who your working with'),
          /// TODO: make this a list of communities that are of type organizations
          if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.isWorkingAlone != null && context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.isWorkingAlone == true)
            ...context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.affiliateOptions.map((e) => Container(
    ...
    return ListTile(
      ...
      subtitle: Row(
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.isOpenToMorePlayers == null || context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.isOpenToMorePlayers == false)
                const Text('Additional Open Slots: Invite Only'),
                if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.isOpenToMorePlayers == true)
                Text('Additional Open Slots: ${context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.attendeeLimit}'),
                const SizedBox(height: 3),
                if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.playerRoster == null || context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.playerRoster?.isNotEmpty == true)
                const Text('Setup Attendees'),
                if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.playerRoster?.isNotEmpty ?? false)
          ...
          if (context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.playerRoster?.isNotEmpty ?? false) Container(
            width: 150,
            child: AvatarStack(
                avatars: context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.classesActivityAvailability?.playerRoster?.map(
...
class ActivityPresetGamePreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      leading: Icon(Icons.library_books_rounded, color: model.paletteColor),
      subtitle: Text('Number of Teams: ${context.read<UpdateActivityFormBloc>().state.activityCreatorForm.activityAvailability.gameActivityAvailability?.tournamentNumberOfTeams ?? 0}',),
      trailing: Icon(Icons.keyboard_arrow_right_rounded, color: model.paletteColor),
    );
  }
