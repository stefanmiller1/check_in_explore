...
class AddNewReservationSlots extends StatefulWidget {
  ...
  const AddNewReservationSlots({super.key, required this.model, required this.listing, required this.reservations, required this.didSaveReservation});
...
class _AddNewReservationSlotsState extends State<AddNewReservationSlots> {
      ...
      child: Column(
        ...
        children: [
          ...
          spaceOptionsForListingToSelect(
              context,
              widget.model,
              widget.listing.listingProfileService.spaceSetting
                  .spaceTypes.value
                  .fold((l) => [], (r) => r),
              currentSpace: context.read<ReservationFormBloc>().state.currentSelectedSpace,
              currentSpaceOption: context.read<ReservationFormBloc>().state.currentSelectedSpaceOption,
          ...
          selectedCalendarDatesSlotReservations(
              ...
              getClosedDatesForCalendar(
                ...
                widget.listing.listingProfileService
                    .backgroundInfoServices.startEndDate,
              ),
              _calendarController,
          ...
          calendarListOfSelectableReservations(
            ...
            getLiveCalendarList(
              ...
              state: context.read<ReservationFormBloc>().state,
            ),
            getSelectedDates(
  ...
  void checkSelectedSpaceDetail(BuildContext context, List<SpaceOption> spaceOptions, ReservationFormState state) {
    if (spaceOptions.isNotEmpty && state.currentSelectedSpace == null) {
      context.read<ReservationFormBloc>().add(ReservationFormEvent.spaceDetailChanged(spaceOptions[0]));
      if (spaceOptions[0].quantity.isNotEmpty) {
        context.read<ReservationFormBloc>().add(ReservationFormEvent.selectedSizeOptionChanged(spaceOptions[0].quantity[0]));
      }
    }
  }
