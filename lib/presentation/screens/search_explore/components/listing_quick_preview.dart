...
Widget getListingPagingController(BuildContext context, DashboardModel model, double bottomOffset, List<ListingManagerForm> listings, UniqueId selectedListing, {required Function(int) didChangePage}) {

  /// based on selected listing-id & search filter type - search for corresponding reservations
  switch (context.read<ListingsSearchRequirementsBloc>().state.searchType) {
    case SearchListingType.facilities:
      return getListingPageQuickPreview(context, model, bottomOffset, listings, selectedListing, null, didChangePage: (page) => didChangePage(page));
    case SearchListingType.activities:
      return BlocProvider(create: (_) => getIt<ReservationManagerWatcherBloc>()..add(ReservationManagerWatcherEvent.watchReservationsList([selectedListing.getOrCrash()], null, null, [ReservationSlotState.refunded, ReservationSlotState.cancelled, ReservationSlotState.requested])),
        child: BlocBuilder<ReservationManagerWatcherBloc, ReservationManagerWatcherState>(
          builder: (context, state) {
            return state.maybeMap(
              ...
              loadReservationListSuccess: (items) => getListingPageQuickPreview(context, model, bottomOffset, listings, selectedListing, items.item, didChangePage: (page) => didChangePage(page)),
              orElse: () => Container()
          );
        },
      )
...
Widget getListingPageQuickPreview(BuildContext context, DashboardModel model, double bottomOffset, List<ListingManagerForm>? listings, UniqueId selectedListingId, List<ReservationItem>? reservation, {required Function(int) didChangePage}) {
...
Widget getFacilityQuickPreview(BuildContext context, DashboardModel model, ListingManagerForm listing) {
  String? listingImage;
