...
class SettingsListContainer extends StatefulWidget {
  ...
  const SettingsListContainer({Key? key, required this.model, required this.currentSelectedSettingItem, required this.didSelectNavItem, required this.currentReservationItem, this.currentActivityManagerForm}) : super(key: key);
...
class _SettingsListContainerState extends State<SettingsListContainer> {
  ...
  List<SettingsItemModel> updateListWithReservationOptions(BuildContext context) {
    ...
    if (widget.currentReservationItem != null) {
      for (ReservationSlotItem reservationSlot in widget.currentReservationItem!.reservationSlotItem) {
        settingsList.insert(3, SettingsItemModel(settingIcon: getIconDataForActivity(context, reservationSlot.selectedActivityType), sectionNavItem: SettingSectionMarker.basics, navItem: SettingNavMarker.reservation, settingsTitle: '${DateFormat.MMMd().format(reservationSlot.selectedDate)} Activity', settingSubTitle: getTitleForActivityOption(context, reservationSlot.selectedActivityType) ?? '', resSlotItem: reservationSlot));
      }
    }
    return settingsList;
